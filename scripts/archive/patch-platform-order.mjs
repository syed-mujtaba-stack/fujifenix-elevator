...
async function main() {
  ...
  const specGroups = (product.specGroups ?? []).map((g) => {
    if (g._key === 'platform') {
      return { ...g, sectionImages: ['luxury-gold-3', 'luxury-gold-2', 'luxury-gold-1'] }
    }
    return g
  })

  await client.patch(product._id).set({ specGroups }).commit()
  console.log('Done: Platform Home Elevator sectionImages reordered to [luxury-gold-3, luxury-gold-2, luxury-gold-1]')
}
