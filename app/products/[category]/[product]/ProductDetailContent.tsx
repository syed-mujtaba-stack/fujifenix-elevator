...
export default function ProductDetailContent({ product }: { product: ProductData }) {
  ...
  const galleryImages = rawGallery.map((g) => ({
    ...g,
    src: getSafeImageUrl(g.src),
  }));
  ...
  return (
    <>
      <div className="bg-white">
        <div className="container-gutter py-4">
          <nav className="flex flex-wrap items-center gap-2 text-sm" aria-label="Breadcrumb">
            ...
            <Link href="/products" className="text-slate-500 hover:text-[#0047BB] transition-colors">Products</Link>
            <span className="text-slate-300" aria-hidden="true">/</span>
            <Link href={`/products/${product.categorySlug}`} className="text-slate-500 hover:text-[#0047BB] transition-colors">{product.category}</Link>
            <span className="text-slate-300" aria-hidden="true">/</span>
      ...
      <section ref={ref} className="bg-white">
        <div className="container-gutter py-16 md:py-20">
          <div className="pd-body grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div>
              <SectionHeading eyebrow="PRODUCT DETAILS" title="OVERVIEW" description={product.description ?? undefined} />
              {product.category && (
              ...
              {product.features && product.features.length > 0 && (
                <div className="pd-reveal mt-2 flex flex-wrap gap-3">
                  {product.features.map((f) => (
                    <span key={f} className="inline-flex items-center gap-2 bg-[#f1f5f9] text-[#0f172a] px-4 py-2" style={{ fontSize: "12px", letterSpacing: "0.06em" }}>
                      <span className="w-1 h-1 bg-[#0047BB] rounded-full" />
                      {f.split(' — ')[0]}
                    </span>
                  ))}
              ...
              <Link href="/contact" className="pd-reveal mt-12 inline-flex items-center gap-3 eyebrow text-[#0047BB] hover:gap-5 transition-all duration-300">
                REQUEST A QUOTE
                <span>→</span>
              </Link>
            </div>
      ...
      {product.operation && (
        <section className="bg-white">
          <div className="container-gutter py-16 md:py-20">
            ...
            <p className="heading text-[#0f172a] max-w-3xl" style={{ fontSize: "clamp(20px, 2.5vw, 28px)", lineHeight: 1.4 }}>{product.operation}</p>
          </div>
        </section>
      )}
                    ...
                    {group.items && group.items.length > 0 && (
                      <div className="max-w-2xl">
                        <div className="bg-white border border-slate-200 shadow-[0_8px_24px_rgba(15,23,42,0.035)]">
                          <div className="bg-[#0047BB] px-6 py-4"><h3 className="eyebrow text-white">{group.title}</h3></div>
                          <dl>
                            {(group.items ?? []).map((item, i) => (
                              <div key={item._key ?? item.label} className={`flex items-center justify-between gap-6 px-6 py-5 ${i > 0 ? "border-t border-slate-100" : ""}`}>
                                <dt className="eyebrow text-slate-400">{item.label}</dt>
                                <dd className="heading text-[#0f172a] text-right" style={{ fontSize: "17px" }}>{item.value}</dd>
                              </div>
                            ))}
                          </dl>
                        </div>
                ...
                <div className="container-gutter py-12 md:py-16">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {typeGroups.map((group) => (
                      <div key={group._key ?? group.title} className="bg-white border border-slate-200 p-6">
                        <h3 className="text-[#0047BB] font-bold text-sm mb-2">{group.title}</h3>
                        {group.sectionDescription && <p className="text-[#475569] text-[13px] leading-relaxed">{group.sectionDescription}</p>}
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <section className="bg-white">
          <div className="container-gutter py-16 md:py-20">
            <SectionHeading
              ...
              description={!isMovingWalk ? (isEscalator ? "A selection of escalator interior finishes available for this product. Click any image to view it larger." : "A selection of cabin interior finishes available for this product. Click any image to view it larger.") : "Click any image to view it larger."}
            />
        ...
        <section className="bg-white">
          <div className="container-gutter py-16 md:py-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
              ...
              <Link href={`/products/${product.categorySlug}`} className="group inline-flex items-center gap-3 eyebrow text-[#0047BB] hover:gap-5 transition-all duration-300 flex-shrink-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {related.map((p) => (
                <Link key={p._id} href={`/products/${product.categorySlug}/${p.slug}`} className="group block">
                  <div className="relative overflow-hidden aspect-[4/3] bg-[#e2e8f0]">
                    {(p.image || p.gallery?.[0]?.src) ? (
                      <Image
                        src={p.image ? urlFor(p.image).width(800).auto("format").url() : getSafeImageUrl(p.gallery?.[0]?.src)}
                      ...
                      <div className="w-full h-full flex items-center justify-center bg-[#0f172a]">
                        <div className="grid grid-cols-6 gap-3 w-20 opacity-30">
                          {Array.from({ length: 12 }).map((_, j) => <div key={j} className="aspect-square bg-[#cbd5e1] rounded-[2px]" />)}
                        </div>
                      </div>
                    )}
                  ...
                  <div className="pt-4">
                    <h3 className="heading text-[#0f172a]" style={{ fontSize: "17px", letterSpacing: "0.02em" }}>{p.title}</h3>
                    <p className="mt-4 mb-8 border-l-2 border-[#0047BB]/30 pl-4 text-[13px] leading-relaxed text-slate-500">Images are for illustrative purposes only. Actual product may vary according to project requirements and selected specifications.</p>
                  </div>
                </Link>
              ))}
      ...
      {zoom && (
        <div role="dialog" aria-modal="true" aria-label={zoom.alt} className="fixed inset-0 z-[100] bg-[#071324]/95 flex items-center justify-center p-4 md:p-10" onClick={() => setZoom(null)}>
          <button type="button" onClick={() => setZoom(null)} aria-label="Close" className="absolute top-5 right-5 z-10 flex h-11 w-11 items-center justify-center border border-white/30 text-white text-lg transition-colors hover:bg-white hover:text-[#071324]">✕</button>
