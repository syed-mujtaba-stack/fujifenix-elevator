...
import { PRODUCT_CATEGORIES, PRODUCT_CATEGORY_GROUPS } from "@/app/data/content";
    ...
    <section ref={ref} className="bg-white">
      <div className="px-8 md:px-16 lg:px-24 py-24 md:py-32">
        ...
        {PRODUCT_CATEGORY_GROUPS.map((group) => {
          const categories = PRODUCT_CATEGORIES.filter((c) => c.group === group.key);
          return (
            <div key={group.key} className="mt-20 md:mt-24 first:mt-0">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
                <div className="flex items-center gap-4">
                  ...
                  <span className="eyebrow text-slate-400">{String(categories.length).padStart(2, "0")}</span>
                </div>
              ...
              <div className="category-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {categories.map((c) => {
                  ...
                  return (
                    <div key={c.slug} className="category-card group">
